...
// 【核心功能】构造映射主类并初始化模块
// 【输入/输出】输入: ROS NodeHandle; 输出: 完成参数与资源初始化
...
// 【核心功能】读取 ROS 参数并写入配置
// 【输入/输出】输入: NodeHandle; 输出: 填充配置与阈值
void LIVMapper::readParameters(ros::NodeHandle &nh)
{
  ...
  nh.param<int>("visual/image_stride_normal", image_stride_normal, 3);
  nh.param<int>("visual/image_stride_degenerate", image_stride_degenerate, 1);
  nh.param<double>("visual/keyframe_trans_thresh", keyframe_trans_thresh, 1.0);
  nh.param<double>("visual/keyframe_rot_thresh", keyframe_rot_thresh, 30.0);
  nh.param<double>("visual/keyframe_scale_min", keyframe_scale_min, 0.3);
  nh.param<int>("visual/max_points_per_voxel", max_points_per_voxel, 30);
  nh.param<int>("visual/point_max_age", point_max_age, 50);
...
void LIVMapper::initializeFiles() 
{
  ...
  const std::string runtime_log_name = std::string(ROOT_DIR) + "Log/runtime_log_" + GetLocalNowString() + ".txt";
  fout_runtime_log.open(runtime_log_name, std::ios::out);
  if (!fout_runtime_log.is_open())
  {
    std::cerr << "Failed to open runtime log file: " << runtime_log_name << std::endl;
  }
  SetRuntimeLogStream(&fout_runtime_log);
  fout_pre.open(DEBUG_FILE_DIR("mat_pre.txt"), std::ios::out);
  fout_out.open(DEBUG_FILE_DIR("mat_out.txt"), std::ios::out);
}

// 【核心功能】初始化 ROS 订阅与发布
// 【输入/输出】输入: NodeHandle 与 ImageTransport; 输出: 绑定回调
...
// 【核心功能】执行 IMU 传播与点云去畸变
// 【输入/输出】输入: 测量缓存; 输出: 更新状态与点云
...
// 【核心功能】根据模式选择 LIO/VIO 更新
// 【输入/输出】输入: 当前模式; 输出: 更新状态与地图
...
{    
  ...
  {
    bool map_slid = voxelmap_manager->mapSliding();
    if (map_slid)
    {
      double radius = voxelmap_manager->config_setting_.half_map_size *
...
// 【核心功能】基于单步 IMU 传播更新状态
// 【输入/输出】输入: 状态与IMU量测; 输出: 更新状态
...
// 【核心功能】定时器回调用于 IMU 传播里程计
// 【输入/输出】输入: 定时事件; 输出: 发布传播结果
...
// 【核心功能】将雷达点云从机体系转换到世界系
// 【输入/输出】输入: 位姿与点云; 输出: 变换后的点云
...
// 【核心功能】标准点云回调入口
// 【输入/输出】输入: PointCloud2 消息; 输出: 缓存点云数据
...
// 【核心功能】Livox 点云回调入口
// 【输入/输出】输入: Livox 自定义消息; 输出: 缓存点云数据
...
// 【核心功能】将 ROS 图像消息转换为 OpenCV 格式
// 【输入/输出】输入: ROS 图像消息; 输出: cv::Mat 图像
...
void LIVMapper::img_cbk(const sensor_msgs::ImageConstPtr &msg_in)
{
  ...
  // 【步骤】按退化状态与关键帧规则选择图像
  img_counter++;
  bool lidar_degenerate = voxelmap_manager->IsLidarDegenerate();
  double sigma_min = voxelmap_manager->GetSigmaMin();
  double scale = std::max(keyframe_scale_min, std::min(1.0, 3.0 * sigma_min));
  double trans_thresh = scale * keyframe_trans_thresh;
  double rot_thresh = scale * keyframe_rot_thresh;
  bool is_keyframe = true;
  double delta_trans = 0.0;
  double delta_rot = 0.0;
  if (has_keyframe_state_)
  {
    delta_trans = (_state.pos_end - last_keyframe_state_.pos_end).norm();
    M3D dR = last_keyframe_state_.rot_end.transpose() * _state.rot_end;
    double cos_angle = std::max(-1.0, std::min(1.0, 0.5 * (dR.trace() - 1.0)));
    delta_rot = std::acos(cos_angle) * 180.0 / M_PI;
    is_keyframe = (delta_trans > trans_thresh) || (delta_rot > rot_thresh);
  }
  int stride = lidar_degenerate ? image_stride_degenerate : image_stride_normal;
  stride = std::max(1, stride);
  bool stride_hit = (img_counter % stride == 0);
  bool use_image = lidar_degenerate || stride_hit || is_keyframe;
  if (!use_image)
  {
    std::ostringstream log_stream;
    log_stream << "[Image Drop] utc=" << GetUtcNowString() << " img_time=" << std::fixed << std::setprecision(6) << img_time_correct
               << " stride=" << stride << " counter=" << img_counter << " keyframe=" << is_keyframe
               << " degenerate=" << lidar_degenerate << " stride_hit=" << stride_hit
               << " delta_trans=" << delta_trans << " delta_rot=" << delta_rot
               << " trans_thresh=" << trans_thresh << " rot_thresh=" << rot_thresh
               << " has_keyframe_state=" << has_keyframe_state_;
  ...
  {
    const char *use_reason = lidar_degenerate ? "degenerate" : (is_keyframe ? "keyframe" : "stride");
    std::ostringstream log_stream;
    log_stream << "[Image Use] utc=" << GetUtcNowString() << " img_time=" << std::fixed << std::setprecision(6) << img_time_correct
               << " reason=" << use_reason << " stride=" << stride << " counter=" << img_counter
               << " keyframe=" << is_keyframe << " degenerate=" << lidar_degenerate << " stride_hit=" << stride_hit
               << " delta_trans=" << delta_trans << " delta_rot=" << delta_rot
               << " trans_thresh=" << trans_thresh << " rot_thresh=" << rot_thresh
               << " has_keyframe_state=" << has_keyframe_state_;
    WriteRuntimeLog(log_stream.str());
  ...
  if (is_keyframe)
  {
    last_keyframe_state_ = _state;
    has_keyframe_state_ = true;
  }
...
// 【核心功能】发布当前帧彩色图像
// 【输入/输出】输入: 发布器与 VIO 管理器; 输出: ROS 图像消息
    ...
    *pcl_wait_pub += *pcl_w_wait_pub;
    if(pub_num >= pub_scan_num)
    {
      ...
      for (size_t i = 0; i < size; i++)
      {
        ...
        V2D pc(vio_manager->ProjectWorldToImage(p_w));

        if (vio_manager->IsInImage(pc, 3)) // 100
        {
          // 【步骤】从图像采样颜色并输出彩色点
...
// 【核心功能】填充位姿消息字段
// 【输入/输出】输入: Pose/Transform 类型; 输出: 写入位姿
...
// 【核心功能】发布 MAVROS 位姿消息
// 【输入/输出】输入: 发布器; 输出: PoseStamped 消息

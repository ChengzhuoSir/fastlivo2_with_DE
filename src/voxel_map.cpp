...
// 【核心功能】从 ROS 参数加载体素地图配置
// 【输入/输出】输入: NodeHandle; 输出: 更新配置结构体
void loadVoxelConfig(ros::NodeHandle &nh, VoxelMapConfig &voxel_config)
{
  ...
  nh.param<double>("lio/degen_sigma_min", voxel_config.degen_sigma_min_, 0.07);
  nh.param<double>("lio/degen_valid_ratio", voxel_config.degen_valid_ratio_, 0.15);
  nh.param<double>("lio/degen_avg_residual", voxel_config.degen_avg_residual_, 0.12);
  nh.param<int>("lio/degen_count_thresh", voxel_config.degen_count_thresh_, 3);
  ...
  nh.param<int>("mapping/local_map_half_size", voxel_config.half_map_size, voxel_config.half_map_size);
  nh.param<double>("mapping/sliding_thresh", voxel_config.sliding_thresh, voxel_config.sliding_thresh);
}

// 【核心功能】在体素中初始化平面模型与统计量
// 【输入/输出】输入: 点集与平面指针; 输出: 填充平面参数
...
void VoxelMapManager::StateEstimation(StatesGroup &state_propagat)
{
  ...
  for (int iterCount = 0; iterCount < config_setting_.max_iterations_; iterCount++)
  {
    ...
    double avg_residual = total_residual / std::max(1, effct_feat_num_);
    if (iterCount == 0)
    {
      valid_plane_ratio_ = static_cast<double>(effct_feat_num_) / std::max(1, feats_down_size_);
      avg_residual_ = avg_residual;

      Eigen::Matrix3d normal_acc = Eigen::Matrix3d::Zero();
      for (const auto &ptpl : ptpl_list_)
      {
        normal_acc += ptpl.normal_ * ptpl.normal_.transpose();
      }
      Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> es(normal_acc);
      if (es.info() == Eigen::Success)
      {
        Eigen::Vector3d evals = es.eigenvalues();
        double max_eval = std::max(1e-6, evals(2));
        sigma_min_ = evals(0) / max_eval;
      }

      const double degen_sigma_min = config_setting_.degen_sigma_min_;
      const double degen_valid_ratio = config_setting_.degen_valid_ratio_;
      const double degen_avg_residual = config_setting_.degen_avg_residual_;
      const int degen_count_thresh = std::max(1, config_setting_.degen_count_thresh_);
      bool degenerate_now = (avg_residual_ > degen_avg_residual);
      degen_count_ = degenerate_now ? degen_count_ + 1 : std::max(0, degen_count_ - 1);
      lidar_degenerate_ = (degen_count_ >= degen_count_thresh);

      std::ostringstream log_stream;
      log_stream << "[LIO Degenerate] utc=" << GetUtcNowString() << " degen_metric=residual sigma_min=" << std::fixed << std::setprecision(6) << sigma_min_
                 << " valid_ratio=" << valid_plane_ratio_ << " avg_residual=" << avg_residual_
                 << " thresh_sigma=" << degen_sigma_min << " thresh_ratio=" << degen_valid_ratio << " thresh_residual=" << degen_avg_residual
                 << " effct_feat=" << effct_feat_num_ << " downsampled=" << feats_down_size_
                 << " degen_count=" << degen_count_ << " degen_thresh=" << degen_count_thresh << " degenerate=" << lidar_degenerate_;
...
// 【核心功能】发布体素平面可视化
// 【输入/输出】输入: 当前体素地图; 输出: ROS MarkerArray
...
// 【核心功能】发布单个平面 Marker
// 【输入/输出】输入: 平面参数与颜色; 输出: MarkerArray 更新

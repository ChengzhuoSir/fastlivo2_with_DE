...
// 【核心功能】初始化 VIO 管理器的基础状态
// 【输入/输出】输入: 无; 输出: 置空缓存与默认参数
...
// 【核心功能】设置 LiDAR 到相机的外参
// 【输入/输出】输入: 旋转矩阵与平移向量; 输出: 更新外参矩阵
...
// 【核心功能】初始化相机内参与 VIO 所需数据结构
// 【输入/输出】输入: 相机模型与配置; 输出: 完成网格与射线初始化
void VIOManager::initializeVIO()
{
  visual_submap = new SubSparseMap;

  downsample_ratio = std::max(0.1, std::min(1.0, downsample_ratio));
  fx = cam->fx() * downsample_ratio;
  fy = cam->fy() * downsample_ratio;
  cx = cam->cx() * downsample_ratio;
  cy = cam->cy() * downsample_ratio;
  image_resize_factor = cam->scale() * downsample_ratio;

  printf("intrinsic: %.6lf, %.6lf, %.6lf, %.6lf\n", fx, fy, cx, cy);

  width = static_cast<int>(cam->width() * downsample_ratio);
  height = static_cast<int>(cam->height() * downsample_ratio);
...
// 【核心功能】在指定尺度提取图像 patch
// 【输入/输出】输入: 图像与像素坐标; 输出: patch 缓冲
...
void VIOManager::insertPointIntoVoxelMap(VisualPoint *pt_new)
{
  ...
  if (iter != feat_map.end())
  {
    ...
    if (points.size() >= static_cast<size_t>(max_points_per_voxel))
    {
      ...
      log_stream << "[VIO Prune] utc=" << GetUtcNowString() << " reason=max_points voxel=(" << position.x << "," << position.y
                 << "," << position.z << ") max_points=" << max_points_per_voxel << " cur_points=" << points.size()
                                                  ...
                                                  const SE3 &T_cur_ref, const int level_ref, Matrix2d &A_cur_ref)
{
  ...
  const double scale = downsample_ratio;
  V2D px_ref_raw = px_ref / scale;
  V2D du_ref = Eigen::Vector2d(kHalfPatchSize, 0) * (1 << level_ref) / scale;
  V2D dv_ref = Eigen::Vector2d(0, kHalfPatchSize) * (1 << level_ref) / scale;
  V3D f_du_ref(cam.cam2world(px_ref_raw + du_ref));
  V3D f_dv_ref(cam.cam2world(px_ref_raw + dv_ref));
  ...
  V2D px_cur(cam.world2cam(f_cur) * scale);
  V2D px_du_cur(cam.world2cam(f_du_cur) * scale);
  V2D px_dv_cur(cam.world2cam(f_dv_cur) * scale);
  A_cur_ref.col(0) = (px_du_cur - px_cur) / kHalfPatchSize;
  A_cur_ref.col(1) = (px_dv_cur - px_cur) / kHalfPatchSize;
}

// 【核心功能】基于深度与姿态计算仿射矩阵
// 【输入/输出】输入: 像素、深度与位姿; 输出: 仿射矩阵
                                        ...
                                        Matrix2d &A_cur_ref)
{
  ...
  Vector2d px_ref_raw = px_ref / scale;
  Vector2d du_ref = Vector2d(halfpatch_size, 0) * (1 << level_ref) * (1 << pyramid_level) / scale;
  Vector2d dv_ref = Vector2d(0, halfpatch_size) * (1 << level_ref) * (1 << pyramid_level) / scale;
  Vector3d xyz_du_ref(cam.cam2world(px_ref_raw + du_ref));
  Vector3d xyz_dv_ref(cam.cam2world(px_ref_raw + dv_ref));
  xyz_du_ref *= xyz_ref[2] / xyz_du_ref[2];
  xyz_dv_ref *= xyz_ref[2] / xyz_dv_ref[2];
  const Vector2d px_cur(cam.world2cam(T_cur_ref * (xyz_ref)) * scale);
  const Vector2d px_du(cam.world2cam(T_cur_ref * (xyz_du_ref)) * scale);
  const Vector2d px_dv(cam.world2cam(T_cur_ref * (xyz_dv_ref)) * scale);
  A_cur_ref.col(0) = (px_du - px_cur) / halfpatch_size;
  A_cur_ref.col(1) = (px_dv - px_cur) / halfpatch_size;
}

// 【核心功能】根据仿射矩阵生成当前帧 patch
// 【输入/输出】输入: 仿射矩阵与参考图像; 输出: patch 缓冲
...
// 【核心功能】计算两块 patch 的 NCC 相似度
// 【输入/输出】输入: 参考与当前 patch; 输出: NCC 分数
...
// 【核心功能】计算雅可比并更新 EKF 状态
// 【输入/输出】输入: 当前图像; 输出: 更新状态与协方差
...
void VIOManager::generateVisualMapPoints(cv::Mat img, vector<pointWithVar> &pg)
{
  ...
  for (int i = 0; i < pg.size(); i++)
  {
    ...
    V3D pt = pg[i].point_w;
    V2D pc(worldToCam(pt));

    if (isInFrame(pc, border)) // 20px is the patch size in the matcher
    {
      int index = static_cast<int>(pc[1] / grid_size) * grid_n_width + static_cast<int>(pc[0] / grid_size);
...
void VIOManager::updateVisualMapPoints(cv::Mat img)
{
  ...
  for (int i = 0; i < total_points; i++)
  {
    ...
    if (pt == nullptr) continue;
    int age = new_frame_->id_ - pt->obs_.back()->id_;
    if (age > point_max_age)
    {
      float loc_xyz[3];
      for (int j = 0; j < 3; j++)
      {
        loc_xyz[j] = pt->pos_[j] / 0.5;
        if (loc_xyz[j] < 0) { loc_xyz[j] -= 1.0; }
      }
      VOXEL_LOCATION position((int64_t)loc_xyz[0], (int64_t)loc_xyz[1], (int64_t)loc_xyz[2]);
      int last_obs_id = -1;
...
// 【核心功能】更新视觉点的参考 patch 与法向
// 【输入/输出】输入: 平面地图; 输出: 更新视觉点状态
...
// 【核心功能】将参考 patch 投影到当前帧用于可视化
// 【输入/输出】输入: 平面地图; 输出: 输出投影结果
void VIOManager::projectPatchFromRefToCur(const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &plane_map)
{
  ...
  for (int i = 0; i < visual_submap->voxel_points.size(); i++)
  {
    ...
    if (pt->is_normal_initialized_)
    {
      ...
      // Feature* ref_ftr;
      V2D pc(worldToCam(pt->pos_));
      V2D pc_prior(worldToCamPrior(pt->pos_));
...
// 【核心功能】预计算参考帧 patch 的雅可比
// 【输入/输出】输入: 金字塔层级; 输出: 缓存雅可比矩阵
...
// 【核心功能】世界坐标投影到图像像素
V2D VIOManager::ProjectWorldToImage(const V3D &xyz_w) const
{
  return worldToCam(xyz_w);
}

// 【核心功能】判断像素是否在图像范围内
bool VIOManager::IsInImage(const V2D &px, int border) const
{
  return isInFrame(px, border);
}

// 【核心功能】世界坐标到相机像素（考虑降采样）
V2D VIOManager::worldToCam(const V3D &xyz_w) const
{
  return cam->world2cam(new_frame_->T_f_w_ * xyz_w) * downsample_ratio;
}

// 【核心功能】世界坐标到相机像素（IMU 先验）
V2D VIOManager::worldToCamPrior(const V3D &xyz_w) const
{
  return cam->world2cam(new_frame_->T_f_w_prior_ * xyz_w) * downsample_ratio;
}

// 【核心功能】相机坐标到像素（考虑降采样）
V2D VIOManager::frameToCam(const V3D &xyz_f) const
{
  return cam->world2cam(xyz_f) * downsample_ratio;
}

// 【核心功能】像素到相机坐标（考虑降采样）
V3D VIOManager::camToWorld(const V2D &px) const
{
  V2D raw_px = px / downsample_ratio;
  return cam->cam2world(raw_px);
}

// 【核心功能】检查像素是否在图像范围内
bool VIOManager::isInFrame(const V2D &px, int border) const
{
  return px[0] >= border && px[1] >= border && px[0] < (width - border) && px[1] < (height - border);
}

// 【核心功能】导出图像与位姿到 Colmap 格式
// 【输入/输出】输入: 当前帧图像与位姿; 输出: 写入 Colmap 文件
...
// 【核心功能】处理单帧图像并更新视觉里程计
// 【输入/输出】输入: 图像、点云与体素地图; 输出: 更新状态与地图

...
// 【核心功能】格式化 UTC 时间字符串
// 【输入/输出】输入: std::time_t; 输出: yyyyMMdd_HHMMSS 格式
inline std::string FormatUtcTime(std::time_t time_value)
{
  std::tm utc_time;
  gmtime_r(&time_value, &utc_time);
  std::ostringstream oss;
  oss << std::put_time(&utc_time, "%Y%m%d_%H%M%S");
  return oss.str();
}

// 【核心功能】获取当前 UTC 时间字符串
// 【输入/输出】输入: 无; 输出: yyyyMMdd_HHMMSS 格式
inline std::string GetUtcNowString()
{
  return FormatUtcTime(std::time(nullptr));
}

// 【核心功能】格式化本地时间字符串
// 【输入/输出】输入: std::time_t; 输出: yyyyMMdd_HHMMSS 格式
inline std::string FormatLocalTime(std::time_t time_value)
{
  std::tm local_time;
...
// 【核心功能】获取当前本地时间字符串
// 【输入/输出】输入: 无; 输出: yyyyMMdd_HHMMSS 格式
inline std::string GetLocalNowString()
{
  return FormatLocalTime(std::time(nullptr));
}

inline std::ofstream *g_runtime_log_stream = nullptr;
inline std::mutex g_runtime_log_mutex;

// 【核心功能】注册全局运行日志输出流
// 【输入/输出】输入: ofstream 指针; 输出: 更新全局日志指针
inline void SetRuntimeLogStream(std::ofstream *stream)
{
  g_runtime_log_stream = stream;
}

// 【核心功能】写入运行日志文件
// 【输入/输出】输入: 日志字符串; 输出: 追加写入
inline void WriteRuntimeLog(const std::string &message)
{
  if (g_runtime_log_stream == nullptr || !g_runtime_log_stream->is_open()) return;
  std::lock_guard<std::mutex> lock(g_runtime_log_mutex);
  (*g_runtime_log_stream) << message << std::endl;
}
